...
\title{Vision and Image Processing:\\ Camera Geometry etc}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Recap of Camera geometry and the perspective transformation \\[3mm]
  \item Vanishing points and vanishing lines \\[3mm]
%   \item Projections, projective geometry, homographies \\[3mm]
  \item Camera matrices and parameters, Camera calibration \\[3mm]
  \item 3D reconstructions \\[3mm]
  \item Lens distortion \\[3mm]
  \item Shape from focusing, Depth of field
...
% ----------------------------------------------------
\begin{frame}
  \frametitle{Projections, Vanishing points}
  \begin{center}
...
directions of minimal and maximal density change. These points at the
...
% ----------------------------------------------------
\begin{frame}
  \frametitle{From World through Camera to Image coordinates}
  \begin{center}
    \includegraphics[width=0.5\textwidth]{IMAGES/CameraCoordSyst.png}
  \end{center}
  \begin{itemize}
    \item The camera and image coordinate axes are coinciding.
    \item Pixels values are sampled at the image plane according to an
       offset (the principal point) and horizontal and vertical
       sampling rates. Pixels may not be square.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Position of the camera coordinate system with respect to the
    world coordinates system:  translation: 3 parameters,
  \item Orientation of the camera coordinate system with respect to
    the world coordinates system:  rotation: 3 parameters.
  \end{itemize}
\end{frame}
...
% ----------------------------------------------------
 \begin{frame}
   ...
   \item Simple Camera calibration matrix, now extended with
     ...
     $$
   \item ${\bf K}_{3\times 3}$ matrix encoding the homogeneous transformations
...
\item Given 6 points $(x,y)$ projected from 6 known 3D points
         $(X,Y,Z)$ we can solve for the camera matrix elements $m_{ij}$. \\[3mm]
...
\item From the 12 elements $m_{ij}$ it is possible to recover all 11 
          parameters $f_x$, $f_y$, $\alpha$, $\beta$, $\gamma$, etc. \\[3mm]
...
%----------------------------------------------
\begin{frame}[fragile]
\frametitle{Solving $A {\bf U} = {\bf 0}$}
...
%----------------------------------------------
\begin{frame}[fragile]
% \frametitle{Solving $A {\bf U} = {\bf 0}$, --2}

\begin{displaymath}
  A \;{\bf U} \;\;=\;\;  U D V^{\top} \; {\bf U} \;\; = \;\; 0
\end{displaymath}
\vspace{3mm}

We are not interested in the trivial solution ${\bf U = 0}$ but rather
the solution lying in the (right) {\color{blue}{Null-space}} of the matrix
$A$. \\[3mm]

Due to noise all singular values we may have $\sigma_i > 0$.  
If we know that $A$ should be singular may find the closest such
matrix by zeroing out the smallest singular value. \\[3mm]

The singular values in the diagonal matrix $D$ specifies the
{\color{blue}{energy}} contained in each of the dimensions. Usually,
these are sorted in decreasing order. \\[3mm]

we get the solution to $A {\bf U} = {\bf 0}$ as the last column of
$V$. In MATLAB: \\
...
polynomium in radial distance from the principal point. \\[4mm]
...
\begin{minipage}{0.50\textwidth}
    \includegraphics[width=0.65\textwidth]{MyImages/lensdistortion3.jpg}
\end{minipage}
%   \end{tabular}
%  \end{center}
\vspace{4mm}
...
\begin{displaymath}
  D   \; =\; \frac{2acfx(x-f)}{a^2f^2 - c^2x^2} 
       \; \approx \;  \frac{2cx^2}{af}
\end{displaymath}

\begin{itemize}
  \item The depth of field increases with squared distance and
    decreases with aperture and focal length. 
  \item At 1m distance the accuracy may be computet to be around 1\%.
            This is comparable or better to what usually can be obtained by
            methods such as stereo. 
\end{itemize}

For good images we wish $D$ large.  For usage in depth from focusing
we wish $D$ small.
 \end{frame}
...
\item Several images:
      \begin{itemize} 
      \item {\color{green}Photometric Stereo}
...
\item Many images:
      ...
      \item  {\color{blue}Structure from Motion} (a large class 
        of methods)
       \end{itemize}
